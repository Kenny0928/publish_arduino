/* --註解區
    > Go_Farmer_Cam
    >作者: Ken ,2024.05.22
    >開發板資訊:AI Thinker ESP32-CAM
    >esp32程式庫版本:3.2.0
    >功能: ESP32-CAM網絡攝像頭，供App Inventor讀取
    >使用AP模式，創建自己的WiFi網絡
    >串流網址:http://192.168.4.1:81/stream
*/
...
// WiFi網絡設置 (AP模式)
const char* ap_ssid = "SMHS-CAM";      // ESP32創建的WiFi名稱
const char* ap_password = "00000000";      // ESP32創建的WiFi密碼
...
static esp_err_t cmd_handler(httpd_req_t *req){
  ...
  // 返回操作結果
  if(res){
    //Serial.println(req, "OK %s = %d\n", variable, val);
  } else {
    //Serial.println(req, "Failed %s = %d\n", variable, val);
  }
...
void setup() {
  ...
  sensor_t *s = esp_camera_sensor_get();
  if (s) {
    s->set_vflip(s, 1); // 1 表示啟用垂直翻轉, 0 表示禁用
    Serial.println("相機畫面已設置為上下翻轉");
  } else {
    Serial.println("獲取相機傳感器失敗，無法設置垂直翻轉");
  }
  // --- 修改結束 ---
